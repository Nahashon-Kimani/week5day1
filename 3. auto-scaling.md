...
- Name the load balancer and select internet-facing based on hosting.md.
...
- Add key pair (created in hosting.md), network settings, and security group consistent with hosting.md.
...
- Choose the VPC and at least two subnets in different Availability Zones cosinstent with **hosting.md**
- Attach an existing Load balancer created in the Step 3. 
- Set the Group size: minimum 2, desired 2, maximum 4 for a highly available pattern.
- Automatic scaling select **No scaling policy.**
...
- Note the Application load balancer dns and load it on the browser tab to access the website via load balancer. 

## Step 8 - Task: Serve the website via CloudFront. 
 - Perform the tasks in  **4. task.md**